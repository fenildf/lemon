    ...
    for (rp = lemp->rule; rp; rp = rp->next) {
      ...
      for (i = 0; i < rp->nrhs; i++) { // 进入右边文法符号 进行for循环
        ...
        if (s2->type == TERMINAL) { // 遇到右边的终结符,不用说了,终结符就是s1的First结合中的元素
          ...
          if (s1->lambda == B_FALSE) break; // 这时候看左边符号lambda,如果为false,说明他不是一个空串,那么肯定说有变富豪的sirstset已经有元素了。
        } else { // 当不是终结符也不是递归是非终结符自身的话没那就要把 右边符号s2的first集合中的元素加入到左边符号的sirstset集合中去
...
//struct lemon *lemp;
{
  ...
  Configlist_init(); // 初始化config 的list
...
//int dot;
{
  ...
  model.dot = dot;
  cfp = Configtable_find(&model);
  if( cfp==0 ){ // 如果返回0,说明这个项目还没有被安装到项目的hash表中
    cfp = newconfig();
    cfp->rp = rp;
    cfp->dot = dot;
    cfp->fws = SetNew(); // fws 域,放置所有follow集元素的数组
    cfp->stp = 0;        // stp是指向项目所在状态state的指针
    cfp->fplp = cfp->bplp = 0;
    cfp->next = 0;
    cfp->bp = 0;
    *currentend = cfp; // 把处理过的项目config装到currentend指针的尾部
    currentend = &cfp->next;
    *basisend = cfp;
    basisend = &cfp->bp;
    Configtable_insert(cfp);
...
//struct lemon *lemp;
{
  ...
  assert(currentend != 0); // 要求不是空链条
  for (cfp = current; cfp; cfp = cfp->next) {
    rp = cfp->rp;
    dot = cfp->dot;
    if (dot >= rp->nrhs) continue; // nrhs  产生式右边所有符号的总数,既包括非终止符,也包括终止符... 如果已经到达产生式的最右端,
    sp = rp->rhs[dot];
    if (sp->type == NONTERMINAL) {
      if (sp->rule == 0 && sp != lemp->errsym) {
        ErrorMsg(lemp->filename, rp->line, "Nonterminal \"%s\" has no rules.",
                 sp->name);
        lemp->errorcnt++; // 如果它是非终结符,而它又没有产生式(用sp->rule==0来判断),同时它还不是我们预先认定的错误符号。则有误。 【因为右边的非终结符号,起码得在产生式的左边出现一次。】
      }
      for (newrp = sp->rule; newrp; newrp = newrp->nextlhs) { //nextlhs 存放着 正是左边具有相同非终结符的产生式链条
        newcfp = Configlist_add(newrp, 0); // 由sp作为左边符号的每一个产生式,都可以建立一个分割符在0处的项目。这种项目成为 非基本项目、。。。。至此,我们为闭包增加了一条项目
        for (i = dot + 1; i < rp->nrhs; i++) { // TODO 开搞follow集。 // nrhs产生式右边所有符号的总数,既包括非终止符,也包括终止符
          xsp = rp->rhs[i];
          if (xsp->type == TERMINAL) {
            SetAdd(newcfp->fws, xsp->index); // 第一种情况,终结符,直接放入follow集合
            break;
          } else {
            SetUnion(newcfp->fws, xsp->firstset); // 第二种情况,非终结符, 把它的First集合中的所有元素合并到Follow集
            if (xsp->lambda == B_FALSE) break; // 第三种情况,空串产生式,则取 后面一个符号重新分析
          }
        }
...
//char **argv;
{
  ...
  if( rpflag ){ // 这个参数 是命令行传进来的,-g, 打印分析结果的统计情况
    ...
    ** links so that the follow-set can be computed later */
    lem.nstate = 0; // nstate 状态个数
    FindStates(&lem); // TODO 最难最难的代码开始了。、、 寻找第一个状态的基本项目;;寻找LR(0)的所有状态
...
//char *s2;
{
  ...
  progress = 0;
  for(i=0; i<size; i++){
    if( s2[i]==0 ) continue; // 先判断第i个终结符是否已在s2的sirstset集合中。如果s2[i]为0,表示这个终结符不在集合中,不用考虑添加。
...
{
  int x;
  x = a->rp->index - b->rp->index; // 产生式的序号index
